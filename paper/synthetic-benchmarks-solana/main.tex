...
\small Vinnytsia National Technical University\\
\small \texttt{Oleksandr.Khoshaba@gmail.com}}
...
For the empirical pilot reported in \cref{sec:empirical-pilot}, the \texttt{txload-low} and \texttt{txload-medium} profiles were implemented by the shell workload driver \path{scripts/run_article_txload_container_v0_4.sh}. The script parameterises a single transaction workload by profile name, duration, target rate, and output directory. It executes inside the running validator container through \texttt{podman exec}, creates temporary payer and recipient keypairs, funds the payer by a local airdrop, and then submits a sequence of \texttt{solana transfer} commands to the local RPC endpoint. Thus, these profiles are not abstract labels in the results section: they are concrete workload perturbations applied to the validator localnet.

\begin{table}[H]
\centering
\small
\caption{Concrete transaction-load profiles used in the empirical pilot}
\label{tab:txload-profile-implementation}
\begin{tabularx}{\textwidth}{L{2.6cm}L{2.9cm}Y Y}
\toprule
\textbf{Profile} & \textbf{Target input} & \textbf{Transaction semantics} & \textbf{Recorded workload evidence} \\
\midrule
\texttt{txload-low} & 300 s run, target 1 transfer/s. & Sequential native SOL transfers of \texttt{0.000001} SOL from a generated payer to a generated recipient after a local \texttt{10} SOL airdrop to the payer. & Per-transfer CSV rows containing timestamp, sequence number, profile, target rate, exit code, command duration, transaction signature, and per-transfer log path; summary JSON with attempted, successful, and failed transactions. \\
\texttt{txload-medium} & 300 s run, target 2 transfers/s. & The same transfer transaction type and account policy as \texttt{txload-low}, but with a higher target submission rate. & The same CSV, log, and JSON evidence, allowing workload-side success counts to be compared with RPC-level transaction-count deltas. \\
\bottomrule
\end{tabularx}
\end{table}
...
The transaction-load runs were generated by \path{scripts/run_article_txload_container_v0_4.sh}. Each workload run begins by creating two temporary keypairs inside the validator container: a payer and a recipient. The payer receives a local airdrop, after which the script repeatedly invokes \texttt{solana transfer} against \texttt{http://127.0.0.1:8899}. Each attempted transaction transfers \texttt{0.000001} SOL from the payer to the recipient using \texttt{--allow-unfunded-recipient}. Operationally, the workload is therefore a sequence of simple value-transfer transactions. It does not exercise SPL-token instructions, application smart-contract logic, a heterogeneous transaction mix, or a separate read-only RPC-pressure workload. The script records each attempt in \texttt{txload\_metrics.csv}, including the sequence number, profile, configured rate, exit code, elapsed command time, extracted transaction signature, and transfer log path, and writes a \texttt{txload\_summary.json} file with attempted, successful, and failed transaction totals.
...
Copy & Copy article source, PDF, build files, documentation, prompts, and scripts into their target paths. & New or updated files appear under \path{paper/}, \path{docs/research-program/}, and \path{scripts/}. \\
...
and local integration helpers for the synthetic benchmarking
...
profile_version: 1
implementation: scripts/run_article_txload_container_v0_4.sh
purpose: Conservative transaction-load validation profile.
duration_seconds: 300
target_transaction_rate_per_second: 1
medium_profile_target_transaction_rate_per_second: 2
burst_mode: false
transaction_type: native_SOL_transfer
amount_sol: 0.000001
airdrop_sol_to_payer: 10
payer_account_policy: generated_per_run
recipient_account_policy: generated_per_run
sequential_cli_confirmation: true
