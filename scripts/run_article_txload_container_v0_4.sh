...
podman exec "$VALIDATOR_CONTAINER" mkdir -p "$WORKDIR"

podman exec "$VALIDATOR_CONTAINER" \
  solana-keygen new --no-bip39-passphrase --force -o "$PAYER_KEYPAIR" \
  > "$OUT_DIR/logs/keygen_payer.log" 2>&1

podman exec "$VALIDATOR_CONTAINER" \
  solana-keygen new --no-bip39-passphrase --force -o "$RECIPIENT_KEYPAIR" \
  > "$OUT_DIR/logs/keygen_recipient.log" 2>&1

PAYER="$(podman exec "$VALIDATOR_CONTAINER" solana-keygen pubkey "$PAYER_KEYPAIR" | tr -d '\r')"
RECIPIENT="$(podman exec "$VALIDATOR_CONTAINER" solana-keygen pubkey "$RECIPIENT_KEYPAIR" | tr -d '\r')"
...
podman exec "$VALIDATOR_CONTAINER" rm -rf "$WORKDIR" || true
