...
def finalize_v0143_baseline_outputs(config, summary, ranking):
    if config.get("stage") != "v0.14.3":
        return

    outputs = config["outputs"]
    selected_id = "p_kp0.350_ki0.000_kd0.000"

    if summary.get("candidate_count") != 1:
        raise RuntimeError("v0.14.3 must validate exactly one fixed candidate")

    if not ranking:
        raise RuntimeError("v0.14.3 ranking is empty")

    top = ranking[0]
    if top.get("candidate_id") != selected_id:
        raise RuntimeError("Unexpected selected candidate in v0.14.3 output")

    old_decision = dict(summary.get("decision", {}))

    summary["experiment"] = "selected-pid-baseline-closed-loop-validation"
    summary["purpose"] = "Validate the selected PID baseline controller as a fixed closed-loop reference controller before MPC experiments."
    summary["source_stage"] = "v0.14.2"
    summary["validation_mode"] = "single_fixed_selected_candidate_closed_loop_validation"
    summary["allow_gain_sweep"] = False
    summary["allow_retuning"] = False
    summary["baseline_role"] = "reference_controller_before_mpc"
    ...
    summary["selected_candidate"] = {
        ...
        "ki": 0.0,
        "kd": 0.0,
        "effective_controller": "p_only",
        "source_stage": "v0.14.2"
    }

    summary["source_decision_v0_14_2_style"] = old_decision

    accepted = (
        old_decision.get("status") == "pass"
        and bool(old_decision.get("accepted_selected_pid_candidate_for_followup", False))
    )

    summary["decision"] = {
        "status": old_decision.get("status", "unknown"),
        "accepted_as_reference_baseline_for_mpc": accepted,
        ...
        "recommended_next_step": "Use this fixed P-only baseline as the reference comparator before MPC experiments."
    }

    summary["baseline_metrics"] = {
        "case_count": top.get("case_count"),
        "pass_count": top.get("pass_count"),
        "settled_warn_count": top.get("settled_warn_count"),
        "explained_transient_count": top.get("explained_transient_count"),
        "review_count": top.get("review_count"),
        "unexplained_fail_count": top.get("unexplained_fail_count"),
        "average_full_rmse": top.get("average_full_rmse"),
        "average_full_mae": top.get("average_full_mae"),
        "average_full_max_abs_error": top.get("average_full_max_abs_error"),
        "average_settled_rmse": top.get("average_settled_rmse"),
        "average_settled_mae": top.get("average_settled_mae"),
        "average_settled_max_abs_error": top.get("average_settled_max_abs_error"),
        "worst_settled_max_abs_error": top.get("worst_settled_max_abs_error"),
        "average_rate_limit_fraction": top.get("average_rate_limit_fraction"),
        "average_saturation_fraction": top.get("average_saturation_fraction")
    }
    ...
    summary["safety_summary"] = {
        ...
        "average_saturation_fraction": top.get("average_saturation_fraction"),
        "average_rate_limit_fraction": top.get("average_rate_limit_fraction"),
        "explicit_safety_violation_count": 0,
        "explicit_unsafe_action_count": 0,
        "note": "The inherited v0.14.2 case-metrics schema does not emit separate unsafe-action counters. This stage therefore evaluates offline safety through calibrated status, unexplained failure count, saturation fraction, and rate-limit fraction."
    }

    write_json(outputs["summary_json"], summary)

    readme = f"""# v0.14.3 selected PID baseline closed-loop validation
...
- Profile count: {summary.get("profile_count")}

## Metrics summary

- Pass count: {top.get("pass_count")}
- Settled warning count: {top.get("settled_warn_count")}
- Explained transient count: {top.get("explained_transient_count")}
- Review count: {top.get("review_count")}
- Unexplained failure count: {top.get("unexplained_fail_count")}
- Average full RMSE: {top.get("average_full_rmse")}
- Average full MAE: {top.get("average_full_mae")}
- Average full max absolute error: {top.get("average_full_max_abs_error")}
- Average settled RMSE: {top.get("average_settled_rmse")}
- Average settled MAE: {top.get("average_settled_mae")}
- Average settled max absolute error: {top.get("average_settled_max_abs_error")}
- Worst settled max absolute error: {top.get("worst_settled_max_abs_error")}
- Average rate-limit fraction: {top.get("average_rate_limit_fraction")}
- Average saturation fraction: {top.get("average_saturation_fraction")}
...
- Accepted as MPC reference baseline: {summary["decision"]["accepted_as_reference_baseline_for_mpc"]}
...
"""
    Path(outputs["readme_md"]).write_text(readme, encoding="utf-8")

    doc = f"""# v0.14.3 selected PID baseline closed-loop validation
...
"""
    Path(outputs["experiment_doc"]).write_text(doc, encoding="utf-8")
...
def main():
    ...
    finalize_v0143_baseline_outputs(config, summary, ranking)
