...
if [ ! -x scripts/v0.8.0/preflight-v0.8.0-with-rpc-port-forward.sh ]; then
  echo "ERROR: preflight wrapper not found or not executable"
  exit 1
fi

JOB_NAME="$(printf '%s' "$RUN_ID" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9-]/-/g')"
JOB_NAME="m0-${JOB_NAME}"
...
if { [ -d "$RAW_DIR" ] || [ -d "$RUN_DIR" ]; } && [ "${FORCE:-0}" != "1" ]; then
  ...
  echo "Use FORCE=1 only when intentionally rerunning this run_id."
  exit 1
fi

mkdir -p "$RAW_DIR" "$RUN_DIR"

echo "== v0.8.0 M0 baseline run =="
echo "run_id:      $RUN_ID"
echo "rate:        $RATE"
...
COLLECT_TIMEOUT="$((DURATION + 180))"

export RUN_ID RATE DURATION SAMPLE START_TIME_UTC RAW_DIR
...
for name in ("load_profile_commanded.csv", "load_profile_applied.csv"):
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["timestamp_utc", "t_seconds", "target_lambda"])
        for t in range(0, duration + 1, sample):
            ts = start + timedelta(seconds=t)
            w.writerow([ts.strftime("%Y-%m-%dT%H:%M:%SZ"), t, rate])
PY
...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/component: v080-m0-baseline
...
if [ -n "$POD_NAME" ]; then
  ...
  kubectl -n "$NAMESPACE" logs "$POD_NAME" > "$JOB_LOG" 2>&1 || true
else
...
raw_dir = os.environ["RAW_DIR"]
run_dir = os.path.dirname(os.environ["RUN_SUMMARY"])
run_id = os.environ["RUN_ID"]
...
summary = {
    "run_id": run_id,
    "experiment_class": "M0-baseline-steady-state",
    "profile_type": "steady_state",
    "rate": float(os.environ["RATE"]),
    "duration_requested_seconds": int(os.environ["DURATION"]),
...
metadata = {
    ...
    "load_profile": {
        "lambda_min": float(os.environ["RATE"]),
        "lambda_max": float(os.environ["RATE"]),
        "lambda_nominal": float(os.environ["RATE"]),
        "parameters": {
            "profile": "steady",
            "rate": float(os.environ["RATE"]),
            "duration_seconds": int(os.environ["DURATION"]),
            "sample_seconds": int(os.environ["SAMPLE"]),
            "implementation": "collect_csv.py steady"
        },
    ...
    "run_verdict": {
        ...
        "notes": "M0 steady-state baseline run generated using collect_csv.py steady."
    }
}
...
if [ "$JOB_OK" != "true" ] || [ "$COPY_OK" != "true" ]; then
  echo
  echo "ERROR: M0 baseline run failed or output copy failed"
  exit 1
fi

echo
echo "M0 baseline run completed: $RUN_ID"
