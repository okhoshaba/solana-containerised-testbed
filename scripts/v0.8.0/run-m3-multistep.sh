...
case "$RUN_ID" in
  ...
  v0.8.0-M3-MS32-128-64-128-32)
    ...
    ;;
  *)
    echo "ERROR: unsupported M3 run_id: $RUN_ID"
    echo "Allowed:"
    echo "  v0.8.0-M3-MS32-64-128-64-32"
    echo "  v0.8.0-M3-MS32-128-64-128-32"
    exit 2
...
JOB_NAME="$(echo "m3-${RUN_ID}" | tr '[:upper:]' '[:lower:]' | sed -E 's/[^a-z0-9-]+/-/g' | cut -c1-63 | sed -E 's/-+$//')"
...
metadata:
  ...
  namespace: ${NAMESPACE}
  labels:
    app.kubernetes.io/name: solana-controlled-load-m3
...
summary = {
    "run_id": run_id,
    "experiment_class": "M3-multi-step-load-profile",
    "profile_type": "multi_step",
    "levels": levels,
    "level_count": level_count,
    "lambda_min": lambda_min,
    "lambda_max": lambda_max,
...
metadata = {
    ...
    "notes": "M3 multi-step profile generated using collect_csv.py step with more than two levels.",
}
...
if [[ "$JOB_OK" != "true" || "$COPY_OK" != "true" || "$COLLECT_ROWS" -le 0 ]]; then
  echo "ERROR: M3 multi-step run failed: $RUN_ID"
  exit 1
fi

echo "M3 multi-step run completed: $RUN_ID"
